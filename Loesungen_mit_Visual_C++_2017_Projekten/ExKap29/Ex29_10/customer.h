...
class Customer
{
   ...
   public:
     Customer( unsigned long id = 0, const string& n = "NN")
     : pin(id), name(n) {}
...
class CustomerFile
{
   private:
     ...
     fstream file;          // File-Stream der Kundendatei.
   ...
   public:
     CustomerFile( const string& s);       // s : Basisname
     unsigned long getCount() const { return count; }
     bool append( const Customer& cust);
